...
description: 从已批准且摘要确认的生产计划发布冻结 GitHub 和 npm 制品；完成 Claude/Codex marketplace 消费者安装验证；完成 happy end 验证；遇到冲突或不确定远端状态时失败关闭并要求人工介入
---
...
node "$CLI" publish --root <path> --plan <plan-path> \
...
发布成功后可运行 `release-verify`；PARTIAL 仅在人工确认远端状态后进入
`release-reconcile`。
