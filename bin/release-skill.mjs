...
Options:
  ...
  --output <path>  Override prepare/approve output path
  ...
  if (hasJson) {
    ...
    const output = {
      ...
      maturity: {
        ...
        onlinePrepare: 'unavailable/fail-closed',
...
if (command === 'approve') {
  ...
  try {
    const approvalPath = outputPath ?? join(dirname(resolve(planPath)), 'approval-record.json');
    const record = await approvePlan({ planPath, expectedDigest, actor, outputPath: approvalPath });

    if (hasJson) {
      console.log(JSON.stringify({ ...record, approvalPath }, null, 2));
    } else {
