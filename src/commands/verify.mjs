 ...
 * 1. Source run status is PUBLISHED or VERIFIED
...
import { readFile, mkdtemp, rm, mkdir, lstat, realpath } from 'node:fs/promises';
...
import { validatePlan, computePlanDigest } from '../core/plan.mjs';
...
import { VERIFIED } from '../core/state-machine.mjs';
...
export async function runSmokeTest(plan, root, options = {}) {
  ...
  try {
    ...
    for (const { package: pkgName, targetVersion, unitId, smokeBin, smokeArgs, smokeExpectedJson } of npmDistributions) {
      ...
      const installResult = await npmExec.install(packageAtVersion, installDir, installFlags);
...
const defaultNpmExecutor = {
  async install(packageAtVersion, cwd, flags) {
    try {
      await execFile('npm', [
...
export async function verifyRelease(options) {
  ...
  try {
    ...
    // Source run must be PUBLISHED or VERIFIED
    if (sourceRun.status !== 'PUBLISHED' && sourceRun.status !== 'VERIFIED') {
      throw new ReleaseError(
        GATE_FAILED,
        `cannot verify: source run status is "${sourceRun.status}"; expected PUBLISHED or VERIFIED`,
    ...
    await writeRunAtomic(verifyRunPath, verifyRunState);
